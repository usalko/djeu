+function ($, window) {
    ...
    function FilePicker(element, options) {
        ...
        function renderPreview() {
            ...
            $(files).each(function () {
                if (!this.type.match('image.*|application/pdf') ) {
                    return;
                }
                var $thumbnail = null;
                if (this.type.match('image.*')) {
                    $thumbnail = $('<img src="' + this.src + '" title="' + this.name + '" style="max-width: 100%; height: auto"/>');
                }
                if (this.type.match('application/pdf')) {
                    $thumbnail = $('<embed src="' + this.src + '" title="' + this.name + '" style="width: 100%; height: auto"/>');
                }
        ...
        function init() {
            ...
            files = $(values).map(function () {
                ...
                if ($.inArray(fileExtension, ['pdf']) >= 0) {
                    // image
    ...
    var _styles = {
        'default': {
            ui: [
                ...
                { className: 'filepicker-ui', childNodes: [{
                    element: '#{ button }',
            ...
            thumbnail: {
                wrap: '<div style="margin-bottom: 20px; text-align: center; border-radius: 0px"></div>',
                style: "max-width: 100%"
            }
        },
        'bootstrap': {
